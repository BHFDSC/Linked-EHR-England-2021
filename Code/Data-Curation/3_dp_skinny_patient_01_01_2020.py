...
# MAGIC %md
# MAGIC **Description** Populates the table `.dp_skinny_patient_01_01_2020` with the following for **each patient alive** on **1st Jan 2020**:
# MAGIC - SEX
# MAGIC - RAW_ETHNICITY
...
# MAGIC 
# MAGIC The records are taken from the `.patient_skinny_unassembled` which is made in notebook `methods/descriptive_paper/data/sam_h_skinny_record`. This is simply a list of all the records for each patient from all the datasets.
# MAGIC  
# MAGIC **Project(s)** All
# MAGIC  
...
# MAGIC  
# MAGIC **Data input** [**table**: `.patient_skinny_unassembled` (from the notebook `descriptive_paper/data/sam_h_skinny_record`)]
# MAGIC 
# MAGIC **Data output** `.dp_skinny_patient_01_01_2020`
# MAGIC 
...
FROM .dp_patient_skinny_unassembled""")
...
# MAGIC %sql
# MAGIC DROP TABLE IF EXISTS .dp_patient_fields_ranked_pre_cutoff
...
# MAGIC       pres.hes_op,
# MAGIC       pres.hes_ae
# MAGIC FROM (SELECT DISTINCT NHS_NUMBER_DEID FROM .dp_patient_fields_ranked_pre_cutoff) pat 
# MAGIC         LEFT JOIN (SELECT NHS_NUMBER_DEID, ETHNIC FROM .dp_patient_fields_ranked_pre_cutoff WHERE ethnic_recency_rank = 1) eth ON pat.NHS_NUMBER_DEID = eth.NHS_NUMBER_DEID
# MAGIC         LEFT JOIN (SELECT NHS_NUMBER_DEID, SEX FROM .dp_patient_fields_ranked_pre_cutoff WHERE sex_recency_rank = 1) sex ON pat.NHS_NUMBER_DEID = sex.NHS_NUMBER_DEID
# MAGIC         LEFT JOIN (SELECT NHS_NUMBER_DEID, DATE_OF_BIRTH FROM .dp_patient_fields_ranked_pre_cutoff WHERE birth_recency_rank = 1) dob ON pat.NHS_NUMBER_DEID = dob.NHS_NUMBER_DEID
# MAGIC         LEFT JOIN (SELECT NHS_NUMBER_DEID, DATE_OF_DEATH FROM .dp_patient_fields_ranked_pre_cutoff WHERE death_recency_rank = 1) dod ON pat.NHS_NUMBER_DEID = dod.NHS_NUMBER_DEID
# MAGIC         LEFT JOIN .dp_patient_dataset_presence_lookup pres ON pat.NHS_NUMBER_DEID = pres.NHS_NUMBER_DEID
...
# MAGIC %sql
# MAGIC -- TRUNCATE TABLE .DP_skinny_patient_01_01_2020_4;

# COMMAND ----------

# cols = ", ".join(spark.table('.DP_skinny_patient_01_01_2020_4').columns)
# spark.sql(f"""INSERT INTO .DP_skinny_patient_01_01_2020_4
#                   SELECT {cols} FROM global_temp.alive_patients_2020""")

# COMMAND ----------

# MAGIC %sql
# MAGIC -- SELECT COUNT ( DISTINCT NHS_NUMBER_DEID) FROM .DP_skinny_patient_01_01_2020_4

# COMMAND ----------

# %sql
# INSERT INTO .DP_skinny_patient_01_01_2020_4
...
# %sql
# DROP TABLE IF EXISTS .DP_skinny_patient_01_01_2020

# COMMAND ----------

# %sql
# ALTER TABLE .DP_skinny_patient_01_01_2020 OWNER TO 

# COMMAND ----------

# %sql
# DROP TABLE .DP_skinny_patient_01_01_2020_4
...
# %sql
# CREATE TABLE .DP_skinny_patient_01_01_2020 AS
