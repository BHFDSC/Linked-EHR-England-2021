...
# dth_og = spark.table(".dp_deaths_") #global_temp.dp_deaths requires first running master nb
# dth_og = dth_og\
#   .withColumnRenamed('DEC_CONF_NHS_NUMBER_CLEAN_DEID', 'PERSON_ID_DEID')\
#   .filter(col("PERSON_ID_DEID").isNotNull())\
#   .withColumn("REG_DATE_OF_DEATH", to_date("REG_DATE_OF_DEATH", "yyyyMMdd"))\
...
skinny = spark.table(".dp_skinny_patient_01_01_2020")
...
create_table(dth_covid_skinny, table_name="dp_covid_death", database_name="", select_sql_script=None)
...
# mi_stroke = spark.table(".descriptive_MI_Stroke_previous")
...
# diab = spark.table(".covariate_flags_diabetes")
...
# obese = spark.table(".covariate_flags_obesity")
